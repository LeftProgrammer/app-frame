  ...
  <view>
    ...
    >
      <uni-forms-item name="username" label="账号">
        <uni-easyinput placeholder="请输入账号" v-model="state.model.username" :inputBorder="false">
...
<script setup>
  ...
  const state = reactive({
    model: {
      ...
      password: '', // 密码
      rememberMe: true,
      tenantName: 'jinghe-sanjiaoroad-app',
    },
    rules: {
      username: mobile,
      password,
  ...
  async function accountLoginSubmit() {
    ...
    // 提交数据
    const { code, data } = await $api?.system?.sAuthApi?.login(state.model);
