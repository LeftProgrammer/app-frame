...
export default {
  ...
  methods: {
        ...
        .boundingClientRect((data) => {
          ...
          if (this.status && this.isEnd == false) {
            ...
            if (this.type !== "1") {
              ...
              if (
                move_block_left >=
                barArea_offsetWidth - parseInt(parseInt(this.blockSize.width) / 2) - 2
              ) {
                move_block_left =
                  barArea_offsetWidth - parseInt(parseInt(this.blockSize.width) / 2) - 2;
              }
    ...
    //鼠标松开
      ...
      if (this.status && this.isEnd == false) {
        if (this.type !== "1") {
          ...
          $api?.system?.sAuthApi?.reqCheck(data).then((res) => {
            if (res.repCode == "0000") {
              ...
              this.passFalg = true;
              this.tipWords = `${((this.endMovetime - this.startMoveTime) / 1000).toFixed(
                2
              )}s验证成功`;
              setTimeout(() => {
                this.tipWords = "";
    ...
    getPictrue() {
      ...
      $api?.system?.sAuthApi?.getCode(data).then((res) => {
        if (res.repCode == "0000") {
